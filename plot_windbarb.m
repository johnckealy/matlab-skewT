...
feather_sep = sin(feather_ang).*flag_length; % defined so that the
%base of a flag is exactly one separation wide.
...
sense = sign(lat).*ones(size(u)); % check for hemisphere and
%set clockwise/anticlockwise feathers
...
for ibarb = 1:nbarbs
    ...
    elseif nhalf==1 & nflag+nfull==0 % only a 5 m/s feather:
        % place the half-feather in the second feather spot, not at the
        % end:
        barb_patch = [0 0;0 shaft_length;0 shaft_length-feather_sep;half_length.*cos(feather_ang) half_length.*sin(feather_ang)+shaft_length-feather_sep;0 shaft_length-feather_sep]*clockmat;
        %disp(['Barb #',int2str(ibarb),': (rounded) speed = 5; drawing
        %half-feather in from the end']);
    else % all other cases:
        ifeather = 0;
        barb_patch = [0 0;0 shaft_length];
        %disp(['Barb #',int2str(ibarb),': (rounded) speed =
        % ',int2str(speed(ibarb))]);
        for iflag = 1:nflag
    ...
    %disp([' We have ',int2str(nverts),' vertices for the final barb
    %patch']);
    
    % should now have an outline of the barb as if wind was northerly with
    % feathers pointing in correct direction for the hemisphere. Now
    %apply the
    % wind-direction rotation matrix:
    barb_patch = barb_patch*windrotn;
    %disp([' Rotated the patch for wind direction of
    %',int2str(round(azmth(ibarb).*180./pi)),' degrees']);
